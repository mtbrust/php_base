 ...
 * 
 */
class bills extends \controllers\EndPoint
{
  ...
  public function get($params)
  {
    ...
    self::$params['response'] = \classes\ApiVindi::getBillsSubscriptions($response['$_GET']['subscription_id'], $response['$_GET']['status']);
  }

  /**
   * getbill
   * 
   * Função retorna uma bill.
   *
   * @param  mixed $params
   * @return void
   */
  public function code($params)
  {
    ...
    $response['code'] = $params['infoUrl']['attr'][1];
    $response['bills'] = \classes\ApiVindi::getBillsCode($response['code'])['bills'];
  ...
  public function post($params)
  {
    ...
    self::$params['response'] = \classes\ApiVindi::postBills($response['$_POST']);
  }
  ...
  public function delete($params)
  {
    ...
    $response['code'] = $params['infoUrl']['attr'][0];
    $bills = \classes\ApiVindi::getBillsCode($response['code'])['bills'];

    // Verifica se não encontrou uma fatura e finaliza.
    if(!isset($bills[0]['id'])){
      $response['error']  = true;
      $response['result'] = "Fatura não existe";
      self::$params['response'] = $response;
      return false;
    }

    // guarda fatura no retonro.
    $response['bills'] = $bills;
    $response['billsId'] = $bills[0]['id'];

    // Caso já tenha sido cancelado.
    if ($bills[0]['status'] == 'canceled') {
      $response['result'] = $bills;
      self::$params['response'] = $response;
      return true;
    }

    // Verifica se a fatura não foi paga.
    if ($bills[0]['status'] == 'paid') {
      $response['error']  = true;
      $response['result'] = "Fatura paga";
    } else {
      $response['result'] = \classes\ApiVindi::deleteBill($response['billsId']);
    }

    // Retonra as informações.
    self::$params['response'] = $response;
  }
}
