 ...
 */
class CalendarioControllerPage extends ControllerPage
{
  ...
  public function pre()
  {
    ...
    $this->paramsView = array(
      'title'            => 'Calendário',            // Título da página exibido na aba/janela navegador.
  ...
  public function delete()
  {
      ...
      //$this->paramsPage['eventos_id'] = $this->attr[1];
    //$this->paramsPage['eventos'] = BdEventos::selecionaPorId($this->attr[1]);
    BdEventos::deleta($this->attr[1]);
  ...
  public function listaEventos(){
      ...
      $params['eventos'] = BdEventos::selecionaTudo();
      $this->paramsPage['eventos'] = $params['eventos'];
      //$eventos = $this->paramsPage['eventos'];
      $i = 0;
      foreach ($params['eventos'] as $value){
          $eventos[$i]['title'] = $value['titulo'];
          $eventos[$i]['start'] = $value['dtIni'];
          $eventos[$i]['end'] = $value['dtFim'];
          $eventos[$i]['color'] = $value['cor'];
          $eventos[$i]['id'] = $value['id'];
          $i++;
      }
      $this->paramsTemplateObjs['list_events'] = json_encode($eventos);


  }

  public function cadastraEvento(){
      if (!empty($_POST['cadEvento'])){
  ...
  public function editaEvento(){
      if (!empty($_POST['editaEvento'])){
          $dateIni = $_POST['start'];
          $dateFim = $_POST['end'];
          $dateIni[10] = " ";
          $dateFim[10] = " ";
          $id = $_POST['editaEvento'];
          $fields = [
              'titulo' => $_POST['title'],
              'cor' => $_POST['color'],
              'dtIni' => $dateIni,
  ...
  public function deletaEvento(){
      ...
      if (!empty($id)){
          ...
          $deleta = $conn->prepare($query);

          $deleta->bindParam("id", $id);
          $deleta->execute();
      }
