 ...
 */
class StatusControllerPage extends ControllerPage
{
  ...
  public function pre()
  {
    ...
    $this->paramsView = array(
      'title'            => 'Status',            // Título da página exibido na aba/janela navegador.
  ...
  public function get()
  {
    ...
    $params['dados'] = BdStatus::selecionaTudo();
    // Monta as colunas.
  ...
  public static function adicionar()
  {
    ...
    if (!empty($_POST['adicionar'])) {
      ...
      if (!empty($_POST['f-nome']) && !empty($_POST['f-statusGrupo'])) {

        // Monta os campos e os valores.
        $fields = [
          'nome' => $_POST['f-nome'],
          'obs' => $_POST['f-obs'],
          'statusGrupo' => $_POST['f-statusGrupo'],
          'idStatusPai' => $_POST['f-idStatusPai'],
          'dtCreate' => date("Y-m-d H:i:s"),
        ];

        // Executa o INSERT.
        $id = BdStatus::Adicionar($fields);
        if (!$id)
          return false;
      }
